// Convollution function where all memeory is stored in global off chip memory. This is a very slow implementation. OP/B = 0.25.
...
int main(void) {
    ...
    int size_F = rows_f * cols_f * sizeof(float);
    float *d_N, *d_F, *d_P;
    checkCudaError(cudaMalloc((void**)&d_N, size_N), "Failed to allocate device memory for d_N");
    checkCudaError(cudaMalloc((void**)&d_F, size_F), "Failed to allocate device memory for d_F");
    ...
    checkCudaError(cudaMemcpy(d_F, h_F, size_F, cudaMemcpyHostToDevice), "Failed to copy data from host to device for d_F");

    dim3 dimGrid(rows_N / 4, cols_N / 4, 1);
    dim3 dimBlock(4, 4, 1);
    convolution_2D_basic_kernel<<<dimGrid, dimBlock>>>(d_N, d_F, d_P, r, rows_N, cols_N);
    ...
    checkCudaError(cudaFree(d_F), "Failed to free device memory for d_F");
